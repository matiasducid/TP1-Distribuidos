...
static void escribir2 (const char *to,  int cbytes)
{
	RD rd;
	FILE *f;
	...
	//ACA PONGO LA LLAMADA A LA FUNCION ESCRIBIR QUE HAGA TODO EL MIDDLEWARE.
	remote_write(s,O_CREAT | O_WRONLY | O_APPEND, to, cbytes);
}

static void leer2(char *from, int binicio, int cbytes)
{
	RD rd;
	FILE *f;
	int qty,c_restantes,p_actual;
	...
	to = "alfa.txt";
	printf("transfieirendo %s remoto al %s local\n", from, to);
	//ACA PONER REMOTEREAD (REMOTEREAD ABRE EL ARCHIVO, LO LEE, ESCRIBE EN EL .TXT Y CIERRA EL ARCHIVO REMOTO).
	// REMOTEREAD DEVOLVERIA LA CANTIDAD DE BYTES ESCRITOS, O VER QUE DEVUELVE.
	remote_read(s,O_RDONLY, from, binicio, cbytes);
}
...
void main( void )
	...
	if (orden[0] == 'l' || orden[0] == 'L')
	  ...
	  leer2(nombrearchivo, atoi(byteinicial), atoi(cantidadbytes)); 
	  }
	else
	    //Llamo la escritura. El buffer es local a la función
	    //escribir(nombrearchivo,  atoi(cantidadbytes));
		escribir2(nombrearchivo,  atoi(cantidadbytes));
	}
    }
